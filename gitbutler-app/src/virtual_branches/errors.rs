...
pub enum MoveCommitError {
    #[error("destination branch contains hunks locked to the head commit")]
    DestinationLocked,
...
impl From<MoveCommitError> for crate::error::Error {
    fn from(value: MoveCommitError) -> Self {
        match value {
            MoveCommitError::DestinationLocked => Error::UserError {
                message: "Destination branch contains hunks locked to the head commit".to_string(),
                code: crate::error::Code::Branches,
            },
