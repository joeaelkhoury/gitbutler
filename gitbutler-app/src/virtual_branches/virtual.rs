...
pub fn move_commit(
    ...
    let (source_branch, source_status) = applied_statuses
        .iter_mut()
    ...
    let is_locked = source_branch_non_comitted_files
        .iter()
        .any(|(path, hunks)| {
    ...
    // 2. stash all the changes from the source branch
    ...
    // reset the source branch to the parent commit
    {
        let mut source_branch = branch_reader
            .read(&source_branch.id)
            .context("failed to read branch")?;
        source_branch.head = source_branch_head_parent.id();
        branch_writer.write(&mut source_branch)?;
    }

    {
        ...
        // update the destination branch with the changes from the source branch
